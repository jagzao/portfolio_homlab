...
- Coding: implementar directamente o Task a `general`. Review independiente: `code-reviewer` en contexto fresco. Visual: `visual-reviewer`. Performance: `performance-reviewer`. Arquitectura excepcional: escalar a `kimi-k2.7-code` (premium, escalamiento explícito), volver a Flash después.
...
- Registrar costo/modelo por task en `.agents/session/cost-log.md` si el runtime lo produce; si no, en el handoff. Modelo premium nunca queda como default.
