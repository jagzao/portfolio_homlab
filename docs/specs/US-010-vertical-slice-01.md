...
Status: `READY` — implementation complete, mandatory performance gate open
Accepted by: Juan
Accepted on: 2026-09-02
Last transition: `ACCEPTED → IMPLEMENTED` on 2026-09-03; reconciled `IMPLEMENTED → READY` on 2026-09-04 (see State History)

> **Honest reconciliation note (2026-09-04).** Per External Audit `P0-01`, `US-010` must not remain `IMPLEMENTED` while a mandatory accepted performance gate is still open. All `P0` code fixes from `docs/audits/AUDIT-2026-09-04-pr4-ui-alpha.md` are now **implemented** (WebGL context-loss, deep-link, movement/input model, M5 scope, M7 visual fidelity, Zavit purposeful activity, CI, subagents, security assertion, evidence), and the previously-missing performance measurements now exist (5-minute-route heap +5.3MB delta, GPU/texture estimate 0MB textures primitives-only, Web Vitals LCP 96ms / CLS 0.000, long tasks, INP proxy). **However the desktop p95 frame-time gate (`<= 20 ms`) remains MARGINAL at 18.8-21.0 ms across samples** — over budget, not a PASS. Because `IMPLEMENTED` requires "implementation and applicable internal validation complete," and a mandatory performance gate is still open, the honest status here is `READY` with the code fixes complete. It moves to `IMPLEMENTED`/`AUDITED` only once the desktop frame-time gate is resolved or formally re-accepted with new measured evidence. Do not treat the open frame-time gate as a pass.
...
- [x] On arrival, Zavit is performing a purposeful activity, then detects the visitor and offers Guided Mode or Free Exploration; the choice is skippable (step 4; `docs/vision/ART_DIRECTION.md` Zavit identity). — `Zavit.tsx`, `encounter.ts`, `activity.ts`, `src/experience/zavit/activity.test.ts`, `e2e/zavit.spec.ts` (P0-07 fix: Zavit now performs a visible repair-console idle activity that stops when the greeting fires, with reduced-motion behavior and E2E/visual evidence)
...
- [x] A visitor can return to orientation without restarting, and a direct link can open the Software Lab's semantic content without replaying arrival (`docs/vision/USER_JOURNEY.md` Recovery and alternate paths). — single-page app, `useDeepLinkTarget.ts` (stable `/#software-lab` fragment target with deterministic focus/scroll), `e2e/deep-link.spec.ts` (P0-04 fix: lands on the Software Lab section, focuses the heading, opens the Architecture Table, and works in semantic/WebGL-unavailable mode without entering 3D)

### Cross-cutting

- [x] The chosen movement/input model (above) works in both Guided and Free modes on the desktop test profile. — `WorldScene.tsx`, `PlayerCamera.tsx`, `GuidedControls.tsx`, `e2e/critical-path.spec.ts`, `e2e/zavit.spec.ts`, `e2e/world.spec.ts`, `e2e/movement-input.spec.ts` (P0-02 fix: fixed-spline Guided Mode, drag-to-look in Free Exploration, Escape-opens-semantic-navigation, and skip-stop all now implemented and E2E-tested)
...
- [x] A WebGL-unavailable or WebGL-context-lost condition immediately renders the semantic equivalent with a visible notice; no retry loop blocks content, per `ADR-002`. — `ExperienceBoundary.tsx`, `CanvasErrorBoundary.tsx`, `e2e/smoke.spec.ts`, `e2e/context-loss.spec.ts` (P0-03 fix: real `webglcontextlost` detection and recovery-to-semantic added; E2E induces the context-loss path and proves visible notice, reachable content, and no page reload/trap)
...
- [x] No request from the browser reaches Supabase or any private Second Brain endpoint (network capture proves this; `ADR-005`). — `e2e/critical-path.spec.ts` "never contacts Supabase", `e2e/network-isolation.ts` + `e2e/network-boundary.spec.ts` (strengthened from a string-only `supabase` match to an explicit forbidden host/project allowlist/denylist per P1-02)
...
- **Measured evidence required before this story can be marked `IMPLEMENTED`:** production bundle report (semantic shell JS, 3D runtime chunk), Web Vitals (LCP/INP/CLS) on the mobile 4G profile, 60-second frame-time trace on desktop and mobile profiles, JS heap after a 5-minute route and after 5 enter/exit cycles, GPU texture memory estimate — each compared against `docs/architecture/PERFORMANCE_BUDGET.md` and recorded PASS/FAIL/NOT MEASURED, never assumed.
  ...
  **Honest measurement status (2026-09-04, after P0 fixes):** bundle, Web Vitals (LCP 96ms/CLS 0.000, isolated run — methodology caveat noted), long tasks, INP proxy, 5-minute-route heap (+5.3MB delta, representative multi-pass sample), GPU/texture estimate (0MB textures, primitives-only confirmed), and 5 enter/exit cycles are all now measured. **The desktop 60-second frame-time gate remains MARGINAL:** p95 18.8-21.0 ms across samples against a `<= 20 ms` budget — genuinely over budget, NOT recorded as PASS. This is the one mandatory performance gate still open and is the reason `US-010` is held at `READY` rather than `IMPLEMENTED`.
...
| 2026-09-04 | IMPLEMENTED | READY | project-lead | External Audit `P0-01` found `US-010` marked `IMPLEMENTED` while mandatory accepted performance gates were FAIL/NOT MEASURED. All `P0` code fixes from `docs/audits/AUDIT-2026-09-04-pr4-ui-alpha.md` implemented (P0-02 movement model: spline/drag-to-look/Escape/skip-stop; P0-03 context-loss; P0-04 deep-link; P0-05 M5 stations; P0-06 M7 visual fidelity; P0-07 Zavit purposeful activity; P0-08/09 CI + evidence; P0-10 subagents; P1-02 network assertion) plus the previously-missing measurements (5-min heap +5.3MB, GPU 0MB-textures estimate, Web Vitals, long tasks, INP proxy). Reconciliation: the desktop p95 frame-time gate (`<=20ms`) remains MARGINAL at 18.8-21.0ms across samples — over budget, so `US-010` is not honestly `IMPLEMENTED`. Status set to `READY` with code fixes complete; `IMPLEMENTED`/`AUDITED` require the frame-time gate to be resolved or formally re-accepted with new measured evidence. Re-audit target: `READY FOR EXTERNAL RE-AUDIT — UI ALPHA`. |
